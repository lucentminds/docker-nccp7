...
LABEL maintainer="Scott Johnson"
...
RUN buildDeps='curl gcc make autoconf libc-dev zlib1g-dev pkg-config' \
   && set -x \
   && apt-get update \
   && apt-get install --no-install-recommends $buildDeps --no-install-suggests \
   -q -y gnupg2 dirmngr wget apt-transport-https lsb-release openssh-server \
   openssh-client cron certbot python3-certbot-dns-cloudflare ca-certificates \
...
ADD ./files/cloudflare-api-credentials.ini /root/cloudflare-api-credentials.ini
# Override default nginx welcome page
COPY files/html /usr/share/nginx/html
RUN printf "Html files are located in `/usr/share/nginx/html`\nNginx conf files are in /etc/nginx/conf.d\n" > /root/README.md
RUN chmod 600 /root/cloudflare-api-credentials.ini
